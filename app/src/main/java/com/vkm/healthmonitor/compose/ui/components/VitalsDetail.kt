...
data class VitalsDetail(
    val issues: String,
    val recommendation: String
)

fun buildVitalsSummary(profile: ProfileWithVitals): Map<String, VitalsDetail> {
    ...
    return mapOf(
        "Normal" to VitalsDetail(
            issues = if (normalList.isNotEmpty()) normalList.joinToString(", ") else "None",
            recommendation = "Keep maintaining healthy habits."
        ),
        "Warning" to VitalsDetail(
            issues = if (warningList.isNotEmpty()) warningList.joinToString(", ") else "None",
            recommendation = if (warningRec.isNotEmpty()) warningRec.joinToString(" ") else "Monitor health closely."
        ),
        "Critical" to VitalsDetail(
            issues = if (criticalList.isNotEmpty()) criticalList.joinToString(", ") else "None",
            recommendation = if (criticalRec.isNotEmpty()) criticalRec.joinToString(" ") else "Immediate medical consultation required!"
        )
    )
}
